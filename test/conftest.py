...
def is_pre_ansible214():
    try:
        ...
        if Version(base_version) < Version("2.14"):
            return True
    except pkg_resources.DistributionNotFound:
        pass
...
@pytest.fixture(scope="session")
def skipif_pre_ansible214(is_pre_ansible214):
    if is_pre_ansible214:
        pytest.skip("Valid only on Ansible 2.14+")
